...
The **Alai A2A Agent** lets external agents drive Alai presentations through Google's [Agent2Agent (A2A) protocol](https://github.com/google-a2a) — a standard that lets one agent call another over HTTP+JSON with first-class support for long-running tasks, streaming, and human-in-the-loop pauses.
...
```bash
curl -X POST "https://alai-standalone-backend.getalai.com/a2a/v1/message:stream" \
...
sequenceDiagram
    ...
    Client->>Agent: POST /v1/message:stream (text + presentation_id)
...
| `POST` | `/v1/message:send`             | Send a message and wait for the final task.                                         |
| `POST` | `/v1/message:stream`           | Send a message and stream task events as they happen (recommended).                 |
| `GET`  | `/v1/tasks/{id}`               | Fetch the current state of a task.                                                  |
| `POST` | `/v1/tasks/{id}:cancel`        | Cancel an in-flight task.                                                           |
| `GET`  | `/v1/tasks`                    | List recent tasks (filterable by `context_id`, `status`, `status_timestamp_after`). |

All endpoints except agent-card discovery require `Authorization: Bearer YOUR_API_KEY`. The wire format follows the [A2A protocol spec](https://github.com/google-a2a).
