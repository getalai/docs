...
class AlaiClient:
    ...
    def generate_transcripts(
        ...
        Args:
            presentation_id: ID of the presentation
            slide_ids: Specific slides (None = all slides)
            wait: If True, polls until completion

        Returns:
            Transcripts for each slide
        """
        payload = {}
        if slide_ids:
        ...
        response = requests.post(
            f"{self.base_url}/presentations/{presentation_id}/transcripts",
            headers=self.headers,
            json=payload
        )
        response.raise_for_status()
...
## Example 6: Generate Speaker Notes

Create talking points for your slides to help with presentations.
...
result = client.generate_presentation(
    ...
    - AI can detect cancer earlier than traditional methods
    - 94% accuracy in detecting diabetic retinopathy

    2. Drug Discovery
    - Reduces development time from 10 years to 2-3 years
    - Lower costs and faster trials
...
transcripts = client.generate_transcripts(presentation_id)

print("=== Speaker Notes ===\n")
for slide in transcripts.get("transcripts", []):
    print(f"Slide {slide['slide_order'] + 1}: {slide['title']}")
...
## Example 7: Presentations with Custom Images
